...
export default async function Home() {
  ...
  const isAutheticated = await isAuthenticated() 
  const { data: posts } = await cookieBasedClient.models.Post.list({
    selectionSet: ["title", "id"],
    // overide default authMode  
    authMode: isAutheticated ? "userPool" : "apiKey",
  });
  ...
  return (
    <main className="flex flex-col items-center justify-between p-24 w-1/2 m-auto">
      <h1 className="text-2xl pb-10">List Of All Posts</h1>
      {posts?.map(async (post, idx) => (
        <Post
          onDelete={onDeletePost}
          post={post}
          key={idx}
          isSignedIn={isAutheticated}
        />
      ))}
    </main>
  );
